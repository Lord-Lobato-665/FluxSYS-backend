...
namespace FluxSYS_backend.API.Controllers
{
    ...
    public class CategoriesSuppliersController : ControllerBase
    {
        ...
        [CustomAuthorize("Administrador", "Administrador Empresarial", "Jefe de Departamento", "Subjefe de Departamento", "Colaborador")]
        [HttpGet("get-categories-suppliers-by-company/{companyId}")]
        public async Task<IActionResult> GetCategoriesSuppliersByCompanyId(int companyId)
        {
            try
            {
                var categoriesSuppliers = await _service.GetCategoriesSuppliersByCompanyIdAsync(companyId);
                return Ok(categoriesSuppliers);
            }
            catch (Exception ex)
            {
                await _errorLogService.SaveErrorAsync(ex.Message, ex.StackTrace, "GetCategoriesSuppliersByCompanyIdController");
                return StatusCode(500, "Error interno del servidor.");
            }
        }
